...
import { useResourcesViewModel } from '../hooks/useResourcesViewModel';

const Resources = () => {
  const { state, actions, animations } = useResourcesViewModel();
  const { 
    containerRef, 
    marqueeRef, 
    eventCardRef, 
            ...
            <div className="flex flex-wrap items-end justify-between mb-8 gap-4">
              ...
              <div className="flex bg-[#F0EAD6]/10 rounded-full p-1 backdrop-blur-sm">
                {(["daily", "weekly", "monthly"] as const).map((view) => (
                  <button
                    key={view}
                    onClick={() => actions.setCalendarView(view)}
                    className={`px-4 py-1 text-[10px] font-bold uppercase tracking-widest rounded-full transition-all ${state.calendarView === view
            ...
            <div className="flex flex-col flex-1 min-h-[400px]">
              ...
              {state.calendarView === "daily" && (
                <div className="flex flex-col gap-0 border-l-2 border-[#D4A373]/30 pl-6 ml-3 py-2 animate-fade-in-up">
                  ...
                  {state.data.dailySchedule.map((item, idx) => (
              ...
              {state.calendarView === "monthly" && (
                <div className="bg-[#F0EAD6]/5 border border-[#F0EAD6]/10 rounded-2xl p-6 animate-fade-in-up backdrop-blur-sm">
                  ...
                  <div className="grid grid-cols-7 gap-2">
                    {state.data.monthlyGrid.map((date, idx) => {
                      const events =
                        state.data.monthlyEvents[date as keyof typeof state.data.monthlyEvents];
          ...
          <div className="content-block h-full flex flex-col perspective-1000">
            ...
            >
              <div className="absolute inset-0 transition-opacity duration-500 ease-in-out">
                <img
                  key={state.activeEvent.image}
                  src={state.activeEvent.image}
                  alt={state.activeEvent.title}
                  loading="lazy"
                  decoding="async"
              ...
              >
                <div className="bg-[#D4A373] text-[#2C3628] font-bold text-xs uppercase px-4 py-2 rounded-full mb-4 shadow-lg flex items-center gap-2">
                  <CalendarIcon size={12} /> {state.activeEvent.date}
                </div>
                ...
                <div className="flex items-center gap-2 text-sm font-bold uppercase tracking-widest text-[#D4A373] mb-8">
                  <MapPin size={16} /> {state.activeEvent.location}
                </div>
              </div>

              <div className="absolute top-8 right-8 flex gap-2">
                {state.data.eventsData.map((_, idx) => (
                  <div
                    key={idx}
                    className={`w-2 h-2 rounded-full transition-all ${idx === state.currentEventIndex
                      ? "bg-[#D4A373] w-6"
                      : "bg-[#F0EAD6]/30"
                      }`}
                  />
          ...
          <div className="features-section mt-24 border-t border-[#F0EAD6]/20 pt-16 relative z-10">
            ...
            <div className="feature-carousel overflow-hidden w-full px-4">
              <div
                ...
                style={{
                  transform: `translateX(-${state.featureScrollIndex * 340}px)`,
                }}
              >
                {state.data.partnerArticles.map((article, idx) => (
                  <div
                    key={idx}
